...
def get_shoulder_width(ratio, left_shoulder, right_shoulder):
    ...
    length = left_shoulder['position']['x'] - right_shoulder['position']['x']
    return abs(length) * ratio
...
def get_hip_width(ratio, left_hip, right_hip):
    ...
    # length = math.sqrt(len_squared)
    length = left_hip['position']['x'] - right_hip['position']['x']
    return abs(length) * ratio
...
def process_img(img):
    ...
    image = cv2.imdecode(im_array, flags = cv2.IMREAD_COLOR)
